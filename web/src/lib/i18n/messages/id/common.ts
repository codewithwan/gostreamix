export const commonId = {
  dashboard: "Dasbor",
  streams: "Stream",
  videos: "Video",
  platforms: "Platform",
  settings: "Pengaturan",
  activity: "Aktivitas",
  logout: "Keluar",
  logoutFailed: "Gagal keluar",
  logoutConfirmTitle: "Konfirmasi keluar",
  logoutConfirmDescription: "Tindakan ini akan mengakhiri sesi pada browser ini.",
  logoutConfirmAction: "Ya, keluar",
  language: "Bahasa",
  appearance: "Tampilan",
  notifications: "Notifikasi",
  dark: "Gelap",
  light: "Terang",
  themeSystem: "Sistem",
  menu: "Menu",
  cancel: "Batal",
  close: "Tutup",
  save: "Simpan",
  create: "Buat",
  update: "Perbarui",
  edit: "Ubah",
  add: "Tambah",
  delete: "Hapus",
  refresh: "Segarkan",
  back: "Kembali",
  next: "Lanjut",
  name: "Nama",
  status: "Status",
  show: "Tampilkan",
  hide: "Sembunyikan",
  appTagline: "Pusat kendali stream",
  userMenu: "Menu pengguna",
  userMenuDescription: "Atur tampilan, bahasa, dan sesi akun.",
  authWelcomeBack: "Selamat datang kembali",
  authLoginDescription: "Masuk untuk melanjutkan pengelolaan pipeline siaran.",
  authSignIn: "Masuk",
  authSigningIn: "Sedang masuk...",
  authUsernamePlaceholder: "Nama pengguna",
  authPasswordPlaceholder: "Kata sandi",
  authInvalidCredentials: "Kredensial tidak valid",
  authLoginSuccess: "Berhasil masuk",
  setupTitle: "Setup awal",
  setupDescription: "Buat akun admin pertama untuk membuka GoStreamix.",
  setupEmailPlaceholder: "Email",
  setupConfirmPasswordPlaceholder: "Konfirmasi kata sandi",
  setupButton: "Selesaikan setup",
  setupLoading: "Sedang setup...",
  setupFailed: "Gagal melakukan setup sistem",
  setupSuccess: "Setup selesai. Silakan masuk.",
  platformTypeYoutube: "YouTube",
  platformTypeTwitch: "Twitch",
  platformTypeFacebook: "Facebook",
  platformTypeTiktok: "TikTok",
  platformTypeCustom: "Kustom",
  authForgotPassword: "Lupa kata sandi?",
  authForgotTitle: "Atur ulang kata sandi",
  authForgotDescription: "GoStreamix bersifat self-hosted, jadi reset kata sandi dijalankan di server demi keamanan. Jalankan salah satu perintah berikut di tempat GoStreamix terpasang, lalu ikuti petunjuknya.",
  authForgotProduction: "Produksi (Docker)",
  authForgotBinary: "Produksi (biner)",
  authForgotDev: "Pengembangan",
  authForgotHint: "Ini mengatur ulang akun administrator utama. Tambahkan --set-password='katasandibaru' untuk melewati prompt interaktif.",
  authDemoTitle: "Coba demo langsung",
  authDemoHint: "Klik untuk mengisi otomatis akun demo (hanya-baca), lalu masuk.",
  demoBanner: "Mode demo — hanya-baca. Membuat, mengubah, dan menghapus dinonaktifkan.",
} as const
