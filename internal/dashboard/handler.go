...
func (h *Handler) GetDashboard(c *fiber.Ctx) error {
	u := auth.GetUser(c, h.authSvc)
	if u == nil {
		return c.Redirect("/login")
	}
	return shared.Render(c, pages.Dashboard(u.Username, u.Email, h.getLang(c)))
}
...
func (h *Handler) GetStreams(c *fiber.Ctx) error {
	...
	return shared.Render(c, pages.Streams(u.Username, u.Email, h.getLang(c)))
}
...
func (h *Handler) GetVideos(c *fiber.Ctx) error {
	...
	return shared.Render(c, pages.Videos(u.Username, u.Email, h.getLang(c)))
}
...
func (h *Handler) GetSettings(c *fiber.Ctx) error {
	...
	return shared.Render(c, pages.Settings(u.Username, u.Email, h.getLang(c)))
}

func (h *Handler) GetLogoutConfirm(c *fiber.Ctx) error {
	return shared.Render(c, components.LogoutConfirm(h.getLang(c)))
}
