...
import (
	...
	"github.com/codewithwan/gostreamix/internal/shared"
...
func NewHandler(svc *Service, jwt *JWTService, guard *Guard) *Handler {
...
func (h *Handler) GetSetup(c *fiber.Ctx) error {
	...
	return shared.Render(c, pages.Setup(pages.AuthProps{Lang: getLang(c)}))
}
...
func (h *Handler) PostSetup(c *fiber.Ctx) error {
	...
	if p != cf {
		return shared.Render(c, pages.Setup(pages.AuthProps{Error: "passwords do not match", Lang: lang}))
	}
	if err := h.svc.Setup(c.Context(), u, e, p); err != nil {
		return shared.Render(c, pages.Setup(pages.AuthProps{Error: err.Error(), Lang: lang}))
	}
	return c.Redirect("/login?setup=success")
}

func (h *Handler) GetLogin(c *fiber.Ctx) error {
	return shared.Render(c, pages.Login(pages.AuthProps{Lang: getLang(c)}))
}
...
func (h *Handler) PostLogin(c *fiber.Ctx) error {
	...
	if err != nil {
		return shared.Render(c, pages.Login(pages.AuthProps{Error: "invalid credentials", Lang: lang}))
	}
